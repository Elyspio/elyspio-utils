...
/// </summary>
public abstract class TracingMiddleware : TracingBase, ITracingMiddleware
{
	/// <inheritdoc cref="TracingMiddleware" />
	protected TracingMiddleware(ILogger logger) : base(logger)
	{
	}
		...
		bool logOnExit = true)
	{
		return LogInternal(arguments, LogLevel.Debug, method, fullFilePath, autoExit, logOnExit,
			TracingContext.OpenTelemetryOptions.CaptureCache.Components[CaptureComponent.Middleware]);
	}
}